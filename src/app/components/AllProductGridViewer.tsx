    ...
    return (
        <section className=" grid grid-cols-1 mt-20 lg:grid-cols-3 gap-x-4 w-full gap-y-10 mb-40">
        {
         productData.map((item:singleProductType,index:number) =>{
            return (
               <Link href={`/brief/${item.slug.current}`}><div key={index} className='flex flex-col overflow-hidden mt-8 lg:mt-0 m-8 lg:m-0 hover:scale-110 duration-200'>
                <div className='w-full lg:w-10/12 h-96 bg-gray-300' >
                <Image src={urlForImage(item.image[0])} height={250} width={250} alt={item.productname} className=" h-full w-auto object-center object-cover"/>
                </div>
